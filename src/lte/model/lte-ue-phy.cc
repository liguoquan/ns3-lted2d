...
class D2DReliableHeader : public Header 
{
public:

  D2DReliableHeader ();
  virtual ~D2DReliableHeader ();
  ...
  bool isCachePackeDirty(void);
  uint16_t getCachePacketID(void);

private:
  uint16_t m_data;
};

D2DReliableHeader::D2DReliableHeader ()
{
...
TypeId
D2DReliableHeader::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::D2DReliableHeader")
    .SetParent<Header> ()
    .AddConstructor<D2DReliableHeader> ()
  ;
  return tid;
}
TypeId
D2DReliableHeader::GetInstanceTypeId (void) const
{
  return GetTypeId ();
}

void
D2DReliableHeader::Print (std::ostream &os) const
{
  // This method is invoked by the packet printing
  // routines to print the content of my header.
  //os << "data=" << m_data << std::endl;
  os << "d2d_header_info=" << m_data;
}
uint32_t
D2DReliableHeader::GetSerializedSize (void) const
{
  // we reserve 2 bytes for our header.
  return 2;
}
void
D2DReliableHeader::Serialize (Buffer::Iterator start) const
{
...
uint32_t
D2DReliableHeader::Deserialize (Buffer::Iterator start)
{
...
void 
D2DReliableHeader::SetData (uint16_t data)
{
  m_data = data;
}
uint16_t 
D2DReliableHeader::GetData (void) const
{
  return m_data;
}

// Decode the Header and Determine if the dirty flag is true
bool D2DReliableHeader::isCachePackeDirty (void) {

  const uint16_t MASK = 0xFFFF; // Mask the MSB
  uint16_t value = m_data & MASK;
  value = (value & MASK) >> 11;

  std::cout << "The value of the first byte is " << value << std::endl;
...
// Returns the cache packet unique ID
uint16_t D2DReliableHeader::getCachePacketID (void) {
...
LteUePhy::PhyPduReceived (Ptr<Packet> p)
{
  ...
  /* Edited for NS3  LTE Project */
  if (displayPhyPDUInfo) {
    std::cout<< "========== PHYPDURECEIVED - FUNCTION =============" << std::endl;
    // std::cout<< "------- Max PDU  GetSize Command -------" << std::endl;
    // std::cout << "LTE-UE-PHY.CC (Bytes): " << p->GetSize() << std::endl;
    // std::cout << "Packet UID  : " << p->GetUid() << std::endl;
    std::cout<< "------- Print Command -------" << std::endl;
    p->Print(std::cout);
    std::cout<< "" << std::endl;
    // std::cout<< "------- PrintPacketTags --------" << std::endl;
    // p->PrintPacketTags(std::cout);
    // std::cout<<"" << std::endl;
    std::cout<< "=================================" << std::endl;
  }

  // Receive Phy PDU before custom header is set
  m_uePhySapUser->ReceivePhyPdu (p);


  // std::cout << "Adding Custom Header to the PHY PDU for the Cache Vector" << std::endl;

  // instantiate a header.
  D2DReliableHeader cacheHeader;
  cacheHeader.SetData(0xFFFF);

  // and store my header into the packet.
  p->AddHeader (cacheHeader);

  if (displayPhyPDUInfo) {
    std::cout<< "========== PHYPDUWITHCUSTOMHEADER - FUNCTION =============" << std::endl;
    // std::cout<< "------- Max PDU  GetSize Command -------" << std::endl;
    // std::cout << "LTE-UE-PHY.CC (Bytes): " << p->GetSize() << std::endl;
    // std::cout << "Packet UID  : " << p->GetUid() << std::endl;
    std::cout<< "------- Print Command -------" << std::endl;
    p->Print(std::cout);
    std::cout << "Dirty Bit: " << cacheHeader.isCachePackeDirty() << std::endl;
    std::cout<< "" << std::endl;
    // std::cout<< "------- PrintPacketTags --------" << std::endl;
    // p->PrintPacketTags(std::cout);
    // std::cout<<"" << std::endl;
  ...
  // Caching PDU by adding to the LTE UE Phy vector
  phyCacheVector.push_back(p);

}

void LteUePhy::printCacheVector() {
  for (std::vector<Ptr<Packet>>::iterator it = phyCacheVector.begin() ; it != phyCacheVector.end(); ++it){
    (*it)->Print(std::cout);
    std::cout<< "" << std::endl;
  }
}
...
std::vector<Ptr<Packet>> LteUePhy::getPhyCacheVector() 
{
  return phyCacheVector;
}
