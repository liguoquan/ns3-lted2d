...
public:
  ...
  */
  ...
  // LTE UE PHY Caching Mechanism
  std::vector<Ptr<Packet>> phyCacheVector; // Vector to hold LTE Packets that have been received.
  std::vector<Ptr<Packet>> getPhyCacheVector();

  bool cacheIsFull();
  bool cacheIsEmpty();
  bool sendPacketToCache(Ptr <Packet> p, std::vector <Ptr <Packet>> destinationCache);
  bool removePacketFromCache(Ptr <Packet> p);

  void printCacheVector();
  bool displayPhyPDUInfo = true;
